...
namespace DetiInteract.Guide.Controls.TeacherControl.TeacherItemViewControl
{
	public sealed class TeacherItemViewModel : DependencyObject
	{
		...
		public TeacherItemViewModel(TeacherItem item)
		{
			...
			this.Photo.DecodePixelWidth = 100;
			this.Photo.EndInit();

			this.Office = String.Format("Gab: {0}", item.Office);
			this.Extension = String.Format("Ext: {0}", item.PhoneExt);
			this.WebPage = String.Format("Web: {0}", item.WebPage);
		}
	}
}
